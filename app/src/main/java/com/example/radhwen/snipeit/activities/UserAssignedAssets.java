...
public class UserAssignedAssets extends AppCompatActivity {

    private static final String TAG = UserAssignedAssets.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_user_assigned_assets);

        listView = findViewById(R.id.list_assets_assigned);
        ...
        Call<Asset> call = services.getAssignedAssets(API_KEY, id);

        call.enqueue(new Callback<Asset>() {
            @Override
            public void onResponse(Call<Asset> call, Response<Asset> response) {

                List<Rows> list =  response.body().getRows();

                listView.setAdapter(new UserAssignedAssetsAdapter(UserAssignedAssets.this, list));

                for (Rows rows : list) {
                    Log.d(TAG, "User's Assets: \n" + rows.getName());
                }
            }
            ...
            public void onFailure(Call<Asset> call, Throwable t) {
                ...
                Toast.makeText(UserAssignedAssets.this, "Failed to connect!", Toast.LENGTH_SHORT).show();
            }
        });
    }
}
